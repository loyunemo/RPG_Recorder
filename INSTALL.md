...
数据默认写在项目下的 `data/` 目录，但可以用 `--data-dir` 参数或环境变量换位置
（界面上也能改，见下方常见问题）。
...
**数据存在哪里？怎么备份？**
默认：桌面版在 `%APPDATA%\rpg-recorder\data`，源码版在项目下的 `data/`。
但**你可以把它指到任何地方** —— 界面里「战役笔记 → 数据 → 更换目录…」直接选文件夹即可，
选完会问要不要把现有数据复制过去。最近用过的目录会列在旁边，随时切回来。

启动时也可以用参数指定（优先级高于界面设置）：

```bash
npm run serve -- --data-dir=D:\TRPG\我的战役
npm start -- --data-dir=D:\TRPG\我的战役
```

打包版还可以给快捷方式加参数，或在快捷方式里设 `RW_DATA_DIR` 环境变量。

备份 = 复制整个数据目录；恢复 = 覆盖回去，并在界面里指过来。
界面里点「打开目录」可直接在资源管理器里打开它。

**换目录之后数据会丢吗？**
不会。换目录只是换个「从哪读」，旧目录原样保留。
如果新目录已经有数据，会提示你将读取那里的战役；如果是空目录，会问你复制还是从零开始。
想回去就在「最近使用」里点「切换」。
